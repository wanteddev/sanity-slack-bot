...
⚠️ **워커 수는 메모리에 묶여 있습니다** — 현재 파드는 **4 vCPU / 4GiB 고정**(상향 불가). WebKit은 워커 하나가 웹프로세스만 ~1GB·1코어 이상을 쓰므로 `workers: 3`에서는 합계 ~3.6GB로 한도에 닿아(파드 cgroup `memory.events max` 다수 관측) 페이지가 멈추고 사파리/모바일사파리에서 타임아웃 실패가 무더기로 났습니다. 그래서 `workers: 2` + `timeout: 45s`로 낮춰 **속도보다 안정성을 우선**합니다(브라우저 4종 전체 약 23분 → 28~30분). 그래도 불안하면 `SANITY_WORKERS=1`로 더 내릴 수 있습니다.
